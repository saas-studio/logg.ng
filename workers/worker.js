...
const apiKey = options.logging.api_key
const loggingApiURL = "https://logg.ng/api/cloudflare"
...
async function handleRequest(event) {
  ...
  event.waitUntil(
    addToBatch(loggingEventBody, requestMetadata.cf_connecting_ip, event),
  )
...
const fetchAndSetBackOff = async (lfRequest, event) => {
  if (backoff <= Date.now()) {
    const resp = await fetch(loggingApiURL, lfRequest)
