...
describe("Cloudflare Worker test", () => {
  ...
  it("correctly builds Logg.ng POST request", async () => {
    const logEntry = "log message"
    const metadata = { request: { headers: {} }, response: { headers: {} } }
    const payload = buildLoggingRequest(logEntry, metadata)
  ...
  it("correctly POSTs logs to Logg.ng API", async () => {
    const apiKey = process.env.LOGGING_API_KEY
    const sourceId = process.env.LOGGING_TEST_SOURCE
    const init = {
      method: "POST",
      headers: {
        "X-API-KEY": apiKey,
